...
## 自動判定ルール（apply_auto_reject.js）

企業リストに対して送信可否を自動判定するスクリプト。以下のルールを**優先順位順**に評価し、H列・I列を自動更新する。
...
| 優先度 | 判定条件 | H列 | I列の内容 |
|---|---|---|---|
| 1 | 列L（キーワードHIT）= `×` | `✕` | `【自動判定】キーワード未検出（Webマーケ非該当の可能性）` |
| 2 | 列O（業種カテゴリ）= `業種違い` | `✕` | `【自動判定】業種違い` |
| 3 | 列J（従業員数）≥ 20名 | `✕` | `【自動判定】従業員20名以上` |
| 3 | 列K（資本金）≥ 1,000万円 | `✕` | `【自動判定】資本金1000万円以上` |

> **注意**: すでに `H列=✕ かつ I列に「自動判定」が含まれている行`はスキップ（二重処理防止）
> 既存の `【営業NG】` 等の手動理由がある場合は `/` で連結して追記する

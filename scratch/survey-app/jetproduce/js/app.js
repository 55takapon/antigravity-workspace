...
const CONFIG = {
  ...
  // --- 分岐URL ---
  lowRatingUrl: "./review-guide.html",     // 星1-3 → ご感想フォーム
  highRatingUrl: "./review-guide.html",    // 星4-5 → ご感想フォーム
...
function bindEvents() {
  ...
  // Touch support — prevent double-tap zoom on iOS
...
function handleStarHover(star) {
  if (selectedRating > 0) return; // 選択済みならhoverプレビューしない
...
function handleKeyboard(e) {
  ...
  if (e.key === "ArrowRight" || e.key === "ArrowUp") {
    ...
    if (next > 0) {
      selectedRating = next;
      updateStarDisplay(next);
      updateRatingLabel(next);
      updateSubmitButton();
    }
  }
  ...
  if (e.key === "ArrowLeft" || e.key === "ArrowDown") {
    ...
    selectedRating = prev;
    updateStarDisplay(prev);
    updateRatingLabel(prev);
    updateSubmitButton();
  }
}

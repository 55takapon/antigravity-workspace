...
SERVICE=re.compile(r'(?:ホームページ|[Ww][Ee][Bb](?:サイト)?|ウェブサイト|[Ee][Cc]サイト|ランディングページ)[の・/／企画制作開発保守運用管理、及びやとを\s]{0,12}(?:制作|構築|運用|保守)|(?:SNS|Instagram|インスタグラム|リスティング広告|Web広告|WEB広告|MEO|Googleビジネスプロフィール)[アカウントの・/／広告企画\s]{0,10}(?:運用|代行|支援|対策)')
...
def examine(row):
 ...
 for kind in ('会社概要|会社情報|企業情報|about|company','サービス|事業内容|service|business'):
  for l in links:
   if dk(l['url'])==dk(u) and re.search(kind,l['label']+' '+l['url'],re.I) and l['url'] not in [p.get('url') for p in pages] and not re.search(r'blog|news|column|recruit|\.pdf',l['url'],re.I):
    queue.append(l['url']);break
 for link in dict.fromkeys(queue):
 ...
 for p in valid:
  ...
  for m in SERVICE.finditer(text):
   ...
   if not re.search('学ぶ|学び|学習|略歴|経歴|退職|入社|経験を積|閉鎖|終了しま|事業譲渡|統合しま',snippet):evidence.append({'url':p['url'],'text':snippet})

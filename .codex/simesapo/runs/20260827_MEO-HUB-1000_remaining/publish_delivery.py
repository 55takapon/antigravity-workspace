...
sys.path.insert(0,str(Path(__file__).parent.parent/'20260827_MEO-HUB-1000'))
...
if len(rows)!=832:raise SystemExit('expected exactly 832 additional companies')
...
if len(before)!=6169:raise SystemExit('expected 6168 existing companies')
if before[0]!=FIELDS:raise SystemExit('unexpected target header')
...
print(json.dumps({'append_candidates':len(rows),'requested':1000,'target_met':len(rows)+168==1000,'current_candidates':len(before)-1,'append_recheck':len(em),'apply':a.apply}),flush=True)
...
journal={'candidate_range':f'A{start}:O{end}','exclusion_range':f'A{es}:L{ee}','candidate_rows':matrix,'exclusion_rows':em,'phase':'preflight','target_met':len(rows)+168==1000}
...
if any(pad(g[:15],15)!=pad(w[:15],15) for g,w in zip(back[1:len(before)],before[1:])):raise SystemExit('old rows changed')
...
report={'added':len(rows),'today_added_total':len(rows)+168,'requested':1000,'target_met':len(rows)+168==1000,'status':'TARGET_MET' if len(rows)+168==1000 else 'PARTIAL_TARGET_NOT_MET','total':len(back)-1,'existing_rows_unchanged':True,'candidate_readback_exact':True,'recheck_added':len(em),'exclusion_readback_exact':True,'start_row':start,'end_row':end,'exclusion_start':es,'exclusion_end':ee}

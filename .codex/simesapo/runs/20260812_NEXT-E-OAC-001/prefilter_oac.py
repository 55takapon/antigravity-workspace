...
rows=list(csv.DictReader((RUN/'oac_members_all.data').open(encoding='utf-8-sig')))
...
noncorp=re.compile(r'(大学|専門学校|研究所$|事務所$|協会|工業組合|アダムシナプス$)')
for r in rows:
 key=nn(r['company_name']);d=dom(r['url']);reasons=[]
 if r['member_type']=='private':reasons.append('個人会員')
 if r['member_type']=='supporting' and noncorp.search(r['company_name']):reasons.append('学校・団体・個人事務所')
 if key in names or (d and d in domains) or (r['phone'] and re.sub(r'\D','',r['phone']) in phones):reasons.append('既存または除外リスト一致')
 ...
 if not r['url']:reasons.append('公式URLなし')
...
out=RUN/'oac_prefiltered.data';fields=list(rows[0])

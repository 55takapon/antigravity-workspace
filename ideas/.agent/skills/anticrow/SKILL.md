...
`ideas` ワークスペースでメモ・アイデアを受け取ったときの挙動ルール：

| メッセージの内容 | モード | Discord 返答 |
|---|---|---|
| 通常のメモ（トリガーワード のみ） | **サイレント** | `✅` のみ |
| `応答して` を含む | **応答** | 詳細レポートあり |
| `アイデアまとめ` / `idea-review` | **応答** | digest 生成して返却 |

**サイレントモードの投入先:**

```
C:¥Users¥hangy¥.gemini¥antigravity¥ideas¥inbox¥YYYYMMDD_HHMMSS.md
```

➡️ `ideas/inbox/` はワークスペース内なので **write_to_file の承認不要**。

> ⚠️ **サイレントモードでは `✅` 以外を Discord に返さないこと。**  
> メモのたびに詳細通知が来るのはノイズになるため。

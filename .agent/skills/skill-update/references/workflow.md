...
SKILL.md 本体 1,000トークン以下・scripts/ なし・変更が description / examples / references / 軽微な構造のみ・メタスキル（skill-creator / skill-checker / skill-update）以外、の全条件を満たす場合は、SKILL.md の「軽量パス」節の省略フローを使ってよい。
スナップショット作成と `skill-checker` 全項目判定は軽量パスでも省略してはならない。
...
- ユーザーへの報告前に、報告内容が事実確認済みで推測なしであることを必ず確認する

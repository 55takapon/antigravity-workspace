...
name: skill-update
description: >-
  既存スキル改善専用。description・発火条件・references・examples・knowledge・品質基準・チェック項目の修正、skill-creator/skill-checker等の改善、「このスキル改善して」「精度上げて」「発火しない」で必ず使用。旧版比較・再評価・修正済みスキルを出す。
---
...
> ⚠️ **この環境での運用宣言（最優先・他の記述より優先する）**
> - **cron実行・自動修正モード・feedback日次レビューモードは使用しない（無効）。** 手動改善フロー（ステップ1〜6）のみ採用する。
> - 以下の「cron の基本ルール」「cron のモード分岐」セクションは原文の残置であり、実行してはならない。各ステップ内の「cron 実行時は」「feedback日次レビューモード時は」「自動修正モード時は」の項目はすべて n/a として扱う。
> - 補助スクリプトは Windows 環境のため `python scripts/...` で実行する（`python3` コマンドは存在しない前提）。Python 3 系が必要。
> - 補助スクリプト群はこの環境で未検証。初回実行時はエラーが出たら無理に進めず、状況をユーザーに報告する。
> - 改善作業中に `.agent\skills\` 直下へ `<スキル名>-workspace/` が生成されるが、これはスキルではない。skill-management の「現在のスキル一覧」に載せず、改善完了後は削除してよい。

## 配置とパス解決

正式配置先は `C:\Users\hangy\.gemini\antigravity\.agent\skills\skill-update` とする。
コマンドを実行する時は、必ずこのスキルのルートディレクトリを作業ディレクトリにし、`python scripts/...` の相対パスで実行する。
`common-skills/skill-update/...` や `.codex/skills/...` のような旧配置前提のパスを新規実行に使ってはならない。
対象スキルは `C:\Users\hangy\.gemini\antigravity\.agent\skills\` から探す。
...
以下の適用条件を**全て**満たす場合のみ、省略フローを使ってよい。1つでも外れたらフルフロー（ステップ1〜6）を実施すること。

適用条件（判定結果を報告に必ず書くこと）:
- 対象スキルの SKILL.md 本体が 1,000トークン以下（文字数÷4で概算）
- 対象スキルが scripts/ を持たない
- 変更種別が description / examples / references / 軽微な構造修正のみ（ロジック・自動化・外部API仕様の変更を含まない）
- 対象が skill-creator / skill-checker / skill-update 自身ではない（メタスキルは必ずフルフロー）
- ユーザーがフルフローを明示指定していない

軽量パスの手順:
1. 旧版スナップショットだけは必ず作る（`<スキル名>-workspace/skill-snapshot/` へ対象スキルをコピー）
2. 評価ケースは2件（正常系1+異常系1）に簡略化してよい。`run_iteration.py prepare` / `aggregate_benchmark.py` / `generate_review.py` / `gate` は省略可
3. 新旧の出力を2ケース分並べて比較し、悪化がないことを確認してから修正を適用する
4. 修正後の `skill-checker` は**省略禁止**（フルフローと同じ全項目判定。ここは軽量化しない）
5. 報告に「軽量パス適用」と判定理由（トークン数・scripts有無・変更種別）を必ず書く

軽量パス適用時に限り、ステップ2〜5の補助スクリプト必須項目（prepare / benchmark / review / gate）は n/a として扱う。
...
- 通常の sweep では、最初に `python scripts/automation_manager.py validate-targets` と `python scripts/automation_manager.py prepare-sweep` を必ず実行する
...
- 自動修正モードでは、`python scripts/automation_manager.py select-fix-candidate --policy automation/auto-fix-policy.json` を最初に実行し、候補がなければ `HEARTBEAT_OK` を返して終わる
- 自動修正モードでは、`select-fix-candidate` は固定の `targets.json` だけを見て選んではならない。`auto-fix-policy.json` で許可されたスキル全体から1件を選ぶ
- 自動修正モードでは、候補がある時だけ `python scripts/automation_manager.py create-rollback-point --selection <selection.json>` を実行し、戻しポイントを作ってから修正に進む
- 自動修正モードでは、修正後に `python scripts/automation_manager.py apply-fix --rollback <rollback.json>` と `python scripts/automation_manager.py verify-fix --rollback <rollback.json>` を必ず実行する
- 自動修正モードでは、補助コマンドが失敗した時に `timeout`、`gateway closed` などの一時失敗だけは、`python scripts/automation_manager.py decide-retry --rollback <rollback.json> --message "<失敗文面>" --policy automation/auto-fix-policy.json` で判定し、同じ run 内で最大3回まで再試行してよい
- 自動修正モードでは、構文エラー、許可外変更、ゲート不許可、benchmark 悪化などの致命失敗は1回で止め、`python scripts/automation_manager.py rollback-fix --rollback <rollback.json>` でそのスキルだけ元に戻す
- 自動修正モードでは、3回再試行しても直らない場合は、そのスキルだけ元に戻す
- 自動修正モードでは、最後に `python scripts/automation_manager.py render-fix-report --selection <selection.json> --rollback <rollback.json> --apply <apply_result.json> --verify <verify_result.json>` を実行し、その出力をそのまま返す
- `skill-update/SKILL.md` を読んでいる時は、補助スクリプトを必ず `python scripts/...` で実行する
...
- [ ] feedback日次レビューモード時は `python scripts/feedback_manager.py collect-feedback`、`python scripts/feedback_manager.py build-feedback-candidates`、`python scripts/feedback_manager.py build-feedback-proposals`、`python scripts/feedback_manager.py render-feedback-announcement` を必ず実行した
- [ ] 自動修正モード時は `python scripts/automation_manager.py select-fix-candidate --policy automation/auto-fix-policy.json` を必ず実行した
- [ ] 自動修正モード時は候補がある場合だけ `create-rollback-point` を必ず実行した
- [ ] 自動修正モード時は `automation/auto-fix-policy.json` を前提に、除外対象を避けたことを確認した
- [ ] cron 実行時は `python scripts/automation_manager.py validate-targets` と `python scripts/automation_manager.py prepare-sweep` を必ず実行した
...
- [ ] 報告内容が事実確認済みの情報だけで構成され、推測を混ぜていないことを必ず確認した
...
- [ ] `skill-checker` を通し、報告に推測を混ぜていないか確認したか
...
- 対象外ディレクトリを変更してはならない
- `skill-checker` を省略して完了報告してはならない

## エッジケース

| 状況 | 対応 |
|:-----|:-----|
| 軽量パスの適用条件を全て満たす | 省略フロー（スナップショット+評価2件+skill-checker全項目）で実施し、適用理由を報告する |
| 軽量パスの適用条件を1つでも満たさない | フルフロー（ステップ1〜6）を実施する |

...
> [!CAUTION]
> **【致命的バグ防止ルール】**
> Playwrightによるアクセス時（`page.goto`）に「Navigation to X is interrupted by another navigation to Y」などのエラーが発生した場合、企業名が確認できないため**必ず `true` (スキップ・削除回避)** を返すこと。過去にエラー時に `false` を返していたため、正常な企業を600件以上誤削除する大事故が発生した。「疑わしきは罰せず（アクセスできないサイトは削除しない）」の原則を徹底すること。
...
| 日付 | バージョン | 変更内容 |
|------|-----------|---------|
| 2026-04-29 | 1.8.1 | **[重大バグ修正] `deep_verify_sheet.js` の誤削除バグ修正**: Playwrightの `page.goto` 実行時に「Navigation interrupted」などのエラーが発生した場合、企業名が確認できなかったために `false`（削除）を返していたため、正常な企業を大量削除する事故が発生。エラー時は「安全側に倒す（`true`を返し削除を回避する）」ようロジックを根本修正。 |
...
|---|---|---|---|
| A-01 | Google Sheets にデータを書き込む時 | **書き込み前に必ず最終行を動的取得する。** 固定インデックスでの書き込みは絶対禁止。 | 書き込み前後の行数を比較し、既存データが消えていないことを確認する。 |
| A-02 | リードデータの復旧・移行作業をする時 | **実行前に対象シートのスナップショット（行数・先頭5行・末尾5行）を取得してログに残す。** ※一般則は NG_RULES A-02 として存続。 | 実行後にスナップショットと比較し、意図しない行の消失・移動がないことを確認する。 |
| D-02 | 新規リードをシートに追加する時 | **必ず既存リードとの重複チェック（ドメイン照合）を先に実行する。** exclude_domains.txt との照合なしに追加しない。 | 追加前に duplicate_checker.js を実行し、重複 0 件であることを確認する。 |

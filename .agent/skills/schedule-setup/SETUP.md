...
> 3. **結果はシートの `status` 列で「次に何をすればいいか」が分かる（5バケツ＋色分け）**：
>    - 🟢 **送信済み**（緑）… 完了。何もしなくてよい。
...
>    色分けは初回だけ `python .claude/skills/005-form-send/scripts/tierb_colorize.py <シートキー>` で設定（緑=送信済み/オレンジ=試行後/黄=要見直し/灰=諦め・要手動送信は色なし）。
>

...
function generateRecommendations(data, skipRules = [], targetReviewCount = null) {
  ...
  if (!skip.has('posts')) {
    ...
    const postThresholds = {
      ...
      'クリニック': { min: 2, label: '月2件（2週に1回）' },
      '整体':   { min: 8, label: '月8件=週2回' },
      '施術':   { min: 8, label: '月8件=週2回' },
      '塾':     { min: 2, label: '月2件（2週に1回）' },
      '学習':   { min: 2, label: '月2件（2週に1回）' },
      '士業':   { min: 2, label: '月2件（2週に1回）' },
      '司法書士': { min: 2, label: '月2件（2週に1回）' },
      '税理士': { min: 2, label: '月2件（2週に1回）' },
      '行政書士': { min: 2, label: '月2件（2週に1回）' },
      '弁護士': { min: 2, label: '月2件（2週に1回）' },

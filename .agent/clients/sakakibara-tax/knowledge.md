...
- 士業広告のため、節税額・融資成功・黒字化などの結果を保証する文脈でクエリを使わない。
- キーワードの詰め込み・羅列は禁止。
...
### サービス個別クエリ（比較・検討層。業務内容に対応）

- 税理士 毎月訪問 大阪（巡回監査の差別化。一般の検索語で表現）
- 創業 支援 税理士 大阪
- 相続税 申告 税理士 大阪市中央区（初回面談無料が確認済み）
- 事業承継 相談 税理士 大阪
- 経営計画 策定 支援 税理士
- 決算書 信頼性 融資（書面添付・記帳適時性証明書の文脈で）

### 悩み・状況クエリ（認知層。確認済みのペインに対応）

- 税理士 変更 相談 大阪（「年に一度しか来ない」ペインが確認済み。最重要の乗り換えクエリ）
- 税理士 年一回 しか来ない
- 経営相談 できる 税理士
- 黒字化 支援 税理士（「黒字にします」と保証しない書き方で使う）
- 自計化 とは（TKC・FXシリーズ導入支援の入口）

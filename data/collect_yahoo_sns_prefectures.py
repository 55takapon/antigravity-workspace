...
TERMS = [
    "SNS広告運用会社", "ソーシャルメディア運用会社", "企業SNS運用代行",
    "YouTube運用代行会社", "LINE公式アカウント運用代行", "ショート動画運用代行会社",
    "Instagramマーケティング会社", "採用SNS運用代行",
]


tasks = [(prefecture, term, page) for prefecture in PREFECTURES for term in TERMS for page in range(2)]
found = []
...
with Path("data/sns_yahoo_candidates_wave6.csv").open("w", encoding="utf-8-sig", newline="") as handle:

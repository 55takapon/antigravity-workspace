...
SERVICE_TERMS = [
    ...
    "sns運用", "snsマーケティング", "sns広告", "seo", "販促", "販売促進",
    "集客支援", "web集客", "広告代理", "プロモーション", "ブランディング",
]
PREFS = (
...
def address_of(text):
    pattern = rf"(〒\s*\d{{3}}[-ー−]?\d{{4}}\s*)?(?:{PREFS}).{{2,100}}?(?=(?:TEL|電話|FAX|代表|設立|資本金|事業内容|$))"
...
def verify(seed, names, domains, phones):
    if norm_name(seed["company_name"]) in names or host_of(seed["url"]) in domains:
        return None
    pages = ["", "company/", "about/", "company-profile/", "corporate/", "profile/", "service/", "services/"]
    texts = []
    for path in pages:
    ...
    if not any(term in lower for term in SERVICE_TERMS):
        return None
    non_meo = any(term in lower for term in [
        "webマーケティング", "デジタルマーケティング", "web広告", "広告運用",
        "sns運用", "sns広告", "seo", "販促", "プロモーション", "ブランディング",
    ])
    ...
    return {
        ...
        "phone": phone,
        "maps_url": "",
        "public_source": seed["public_source"],
    }


def main():
    with open("data/round4_public_award_new_seeds.csv", encoding="utf-8-sig") as handle:
    ...
    with open("data/round4_public_award_verified_pre001.csv", "w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(handle, fieldnames=["company_name", "url", "address", "phone", "maps_url", "public_source"])
        writer.writeheader()
        writer.writerows(rows)
    print(f"seeds={len(seeds)} verified={len(rows)}")

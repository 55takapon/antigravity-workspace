...
SOURCE = HERE / "auto_with_contacts.csv"
SEED = HERE / "auto_candidate_seed.csv"
AUDIT = HERE / "auto_preappend_audit.csv"
FINAL = HERE / "auto_final_verified_50.csv"
...
allow_pairs = {(company_key(row.get("company_name", "")), domain_key(row.get("official_domain", "") or row.get("url", ""))) for row in allow if row.get("company_name") and (row.get("official_domain") or row.get("url"))}

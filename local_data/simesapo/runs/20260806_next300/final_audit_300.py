...
result = {"range": f"{TARGET}!A{START}:P{END}", "rows": len(rows), "missing_required": missing, "internal_company_duplicates": internal_company, "internal_domain_duplicates": internal_domain, "external_conflicts": external_conflicts, "snapshot": str(snapshot)}

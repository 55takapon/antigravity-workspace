...
SOURCE = "https://www.joifa.or.jp/list/"
...
def main() -> None:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if not official.startswith("http") or not re.search(r"株式会社|有限会社|合同会社", name):
            continue
        if host(official) in {"joifa.or.jp", ""}:
            continue
        rows.append({
            "company_name": name,
            "url": official,
            ...
            "phone": "",
            "contact_url": "",
            "区分": "H｜オフィス移転・什器・OA・IT環境支援",
            "検出ワード": "日本オフィス家具協会公式会員：オフィス家具・移転・空間・関連設備支援",
            "source_url": SOURCE,
        })
